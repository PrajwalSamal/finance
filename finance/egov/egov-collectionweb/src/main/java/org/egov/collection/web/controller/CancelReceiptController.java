...
@Controller
@RequestMapping(value = { "/cancel/cash_receipt" })
@Validated
public class CancelReceiptController {
    
    private static final String ERROR_MESSAGE = "errorMessage";
	private static final Logger LOGGER = LoggerFactory.getLogger(CancelReceiptController.class);

    public CancelReceiptController() {
    ...
    public String getCancelCashReceiptForm(final Model model, @ModelAttribute(ERROR_MESSAGE) @SafeHtml final String errorMessage) {
    ...
    @SuppressWarnings("rawtypes")
       ...
       public @ResponseBody ResponseEntity getCancelCashReceiptSearch(@Valid @ModelAttribute final DishonoredChequeBean model,final BindingResult errors){
           try {
               ...
               return new ResponseEntity<>(getCancelCashReceipt(model), HttpStatus.OK);
    ...
    private List<DishonoredChequeBean> getCancelCashReceipt(final DishonoredChequeBean model) {
        final String bankBranch = model.getBankBranch();
        String bankId = null;
        if (StringUtils.isNotBlank(bankBranch))
            bankId = bankBranch.split("-")[0].trim();
        return dishonorChequeService.getCollectionListForCancelCashInstrument(model.getInstrumentMode(), bankId,
                model.getAccountNumber(), model.getReceiptNumber(), model.getReceiptDate());
    }

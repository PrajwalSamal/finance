...
@Results(value = { @Result(name = "result", location = "budgetAppropriationRegisterReport-form.jsp"),
...
//    }
        ...
        if (fund.getId() != null && fund.getId() != -1) {
            ...
            if (department.getCode() != null && !("-1".equals(department.getCode()))) {
                ...
                List<String> deptCodeList =
                    budgetDetailService.getDepartmentFromBudgetDetailByFundId(
                            fund.getId());

                if (deptCodeList != null && !deptCodeList.isEmpty()) {
                    for (String deptCode : deptCodeList) {
                        listOfDepartments.add(
                            microserviceUtils.getDepartmentByCode(deptCode));
                    }
                }
	...
	private void generateReport() {
		...
		if (budgetGroup != null) {
			...
			strQuery.append(
					"select vmis.budgetary_appnumber as bdgApprNumber, vh.vouchernumber as VoucherNumber, vh.voucherdate as voucherDate,")
					...
					.append(getDepartmentQuery("vmis.departmentcode")).append(getFundQuery("vh.fundid")).append(" ")
					...
					.append(getFunctionQuery("bd.functionid")).append(getDepartmentQuery("bmis.departmentcode"))
					.append(getFundQuery("bmis.fundid")).append("  ")
					...
					.append(" and br.billdate <=:strAODate").append(getFunctionQuery("bd1.functionid"))
					.append(getDepartmentQuery("bmis1.departmentcode")).append(getFundQuery("bmis1.fundid"))
		...
		if (budgetAppropriationRegisterList.size() > 0) {
			...
			strsubQuery.append(
					"select vmis.budgetary_appnumber as bdgApprNumber, vh.vouchernumber as VoucherNumber, vh.voucherdate as voucherDate, vh.description as description,")
			...
			for (final BudgetAppDisplay budgetAppropriationRegisterDisp : budgetAppropriationRegisterList) {
				if (BudgetingType.DEBIT.equals(budgetGroup.getBudgetingType()))
					if (budgetAppropriationRegisterDisp.getDebitAmount() != null
						...
						budgetAppropriationRegisterDisp.setBillAmount(
								budgetAppropriationRegisterDisp.getCreditAmount().multiply(new BigDecimal("-1")));
				...
				if (BudgetingType.CREDIT.equals(budgetGroup.getBudgetingType()))
					if (budgetAppropriationRegisterDisp.getCreditAmount() != null
						...
						budgetAppropriationRegisterDisp
								.setBillAmount(budgetAppropriationRegisterDisp.getCreditAmount());
						totalCredit = totalCredit.add(budgetAppropriationRegisterDisp.getBillAmount());
					} else {

						budgetAppropriationRegisterDisp.setBillAmount(
								budgetAppropriationRegisterDisp.getDebitAmount().multiply(new BigDecimal("-1")));
				...
				if (BudgetingType.ALL.equals(budgetGroup.getBudgetingType())) {
					if (budgetAppropriationRegisterDisp.getDebitAmount() != null
							&& budgetAppropriationRegisterDisp.getDebitAmount().compareTo(BigDecimal.ZERO) == 1)
						balanceAvailableAmt = totalGrant
								.subtract(budgetAppropriationRegisterDisp.getCumulativeAmount().abs());
					else
	...
	private BigDecimal getBudgetBEorREAmt(final String type) {
		...
		try {
			...
			final CFinancialYear financialYear = financialYearDAO
					.getFinancialYearById(Long.valueOf(financialYr.getId()));
			...
			if (budgedDetailList != null && budgedDetailList.size() > 0)
				for (final BudgetDetail bdetail : budgedDetailList) {
					...
					if ("RE".equalsIgnoreCase(type) && !getConsiderReAppropriationAsSeperate()) {
						...
						addtionalAppropriationForBe = addtionalAppropriationForBe
								.add(bdetail.getApprovedReAppropriationsTotal());
					else {
						shouldShowREAppropriations = true;
						addtionalAppropriationForRe = addtionalAppropriationForRe
								.add(bdetail.getApprovedReAppropriationsTotal());
					}
				}
	...
	public void setBudgetAppropriationRegisterList(final List<BudgetAppDisplay> budgetAppropriationRegisterList) {
	...
	public void setAddtionalAppropriationForBe(final BigDecimal addtionalAppropriationForBe) {
	...
	public void setAddtionalAppropriationForRe(final BigDecimal addtionalAppropriationForRe) {
